def countDirectories(perturbed_ACE_folder_path):
    """
    Count the number of directories within the perturbed ACE files folder of interest

    Parameters:
        perturbed_ACE_folder_path (str): Path to the directory where the folders containing the perturbed ACE files are located

    Results:
        directory_number (int): Number of directories within the folder containing the perturbed ACE files.
        Doesn't include a directory containing the perturbation inputs
    """
    ...
    'Count the number of directories within the folder containing the perturbed ACE files'
    'Doesn''t include directories containing inputs used to create ACE files'
...
def createNumbers(directory_number):
    """
    Create a list of four-digit numbers corresponding to the folders containing each of the perturbed ACE files

    Parameters:
        directory_number (int): Number of directories each containing a perturbed ACE file

    Results:
        four_digit_numbers (list): List of four-digit numbers corresponding to the folders containing the perturbed ACE files
    """
    ...
    for ii in range(0, directory_number):
        ...
        four_digit_numbers.append(str(number))
...
def createUnperturbedLibrary(neutron_sublibrary_path, 
    ...
    formatting used by premade OpenMC data libraries (e.g. H1.h5, U235.h5, etc.)

    Parameters:
        neutron_sublibrary_path (str): Path to the directory where the .h5-formatted
        unperturbed cross section data is located
    ...
    Results:
        ...
        all necessary xs data to run the model of interest except for the 
        perturbed xs data
    """
...
def createModelFolders(directory_number, 
    ...
    Parameters:
        directory_number (int): Number of directories containing perturbed ACE files
    ...
    Results:
        ...
        perturbed_model_folder_list (list): List with the paths to each of the folders with the perturbed cross_sections.xml files
    """
    ...
    'Iterate over the number of directories, create folders to store the cross_sections.xml files, and add their paths to the list'

    for number in range(0, directory_number):

        perturbed_model_folder_name = 'Model_' + str(number+1) + '_Folder'

        os.mkdir(perturbed_model_top_directory_name + '/' + perturbed_model_folder_name)  

        perturbed_model_folder_list.append(perturbed_model_top_directory_name + '/' + perturbed_model_folder_name)

    return perturbed_model_top_directory_name, perturbed_model_folder_list


def createPerturbedXML(unperturbed_library, 
                       perturbed_ACE_folder_path, 
                       four_digit_numbers, 
                       perturbed_model_folder_list):

    """
    Create the cross_sections.xml files with the perturbed xs data that can be used to run the models

    Parameters:
        unperturbed_library (openmc.data.DataLibrary): Data library containing the unperturbed neutron xs data

        perturbed_ACE_folder_path (str): Path to the directory where the perturbed ACE files are located

        four_digit_numbers (list): List containing the list of numbers corresponding to the folders containing the perturbed ACE files

        perturbed_model_folder_list (list): List containing the names of the folders where the perturbed cross_sections.xml files will be stored
    """
    ...
    for ii in range(0, len(perturbed_model_folder_list)):
        ...
        'Grab the name of the folder containing a perturbed ACE files'

        perturbed_data_folder = perturbed_ACE_folder_path + '/' + str(four_digit_numbers[ii]) 
        ...
        perturbed_data = openmc.data.IncidentNeutron.from_ace(Path(perturbed_ACE_file_path))
        ...
        'Might add a modifications to place the .h5 file in the directories where the perturbed xml files are'

        'Register ACE file in xs_libary'
